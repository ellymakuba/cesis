...
if (isset($_POST['submit'])) {
	...
	if ($myrow[0]>0 and !isset($SelectedGradeLevel)) {
		$InputError = 1;
		prnMsg( _('The classes already exists in the database'),'error');
	...
	if( $InputError !=1 ) {
		$ErrMsg = _('The classes could not be inserted or modified because');
		$DbgMsg = _('The SQL used to insert/modify the classes details was');
...
if (isset($SelectedGradeLevel)) {
	echo '<p>';
	echo "<div class='centre'><a href='" . $_SERVER['PHP_SELF'] ."?" . SID . "'>" . _('Show all classes') . '</a></div>';
	echo '<p>';
}
