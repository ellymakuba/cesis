...
/* $Id: PO_SelectOSPurchOrder.php 3944 2010-09-30 15:19:37Z tim_schofield $*/
...
$title = _('Search Outstanding Tenders');
...
if (!isset($OrderNumber) or $OrderNumber=='' ){
	...
	echo '<table class=selection><tr><td>'._('Order Number') . ': <input type=text name="OrderNumber" MAXLENGTH =8 size=9>  ' ;
	
 	echo '<input type=submit name="SearchOrders" value="' . _('Search Tenders') . '"></td></tr></table>';
}
...
//				   '</td><td class="tableheader">' . _('Receive') .
	...
	while ($myrow=DB_fetch_array($PurchOrdersResult)) {
		...
		if ($myrow['status'] == PurchOrder::STATUS_PRINTED) {
			...
			$ReceiveOrder = _('Receive');$ReceiveOrder = "<a href='".$rootpath . "/GoodsReceived.php?" . SID . "&PONumber=" . $myrow["orderno"]."'>".
				_('Receive').'</a>';
		}
