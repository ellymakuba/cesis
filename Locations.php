...
/* $Id: Locations.php 3905 2010-09-30 15:00:30Z tim_schofield $*/
/* $Revision: 1.25 $ */
...
// PREVENT DELETES IF DEPENDENT RECORDS
	...
	if (! $CancelDelete) {
		...
		/* need to figure out if this location is the only one in the same  */
...
or deletion of the records*/
	...
	if (DB_num_rows($result)==0){
		prnMsg (_('There are no locations that match up with a tax  record to display. Check that tax  are set up for all dispatch locations'),'error');
	}
...
if (!isset($_GET['delete'])) {
	...
	echo '<td>' .  _('Tax') . ':' . '</td><td><select name="TaxProvince">';
