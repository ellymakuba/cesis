<?php
/* $Id: TaxProvinces.php 4017 2010-09-30 16:09:22Z tim_schofield $*/

$PageSecurity = 15;

include('includes/session.inc');

$title = _('Dispatch Tax');
...
if (isset($_POST['submit'])) {
	...
	if (strpos($_POST['TaxProvinceName'],'&')>0 OR strpos($_POST['TaxProvinceName'],"'")>0) {
		$InputError = 1;
		prnMsg( _('The tax name cannot contain the character') . " '&' " . _('or the character') ." '",'error');
	}
	if (trim($_POST['TaxProvinceName']) == '') {
		$InputError = 1;
		prnMsg( _('The tax name may not be empty'), 'error');
	}
	...
	if ($_POST['SelectedTaxProvince']!='' AND $InputError !=1) {
		...
		if ( $myrow[0] > 0 ) {
			$InputError = 1;
			prnMsg( _('The tax cannot be renamed because another with the same name already exists.'),'error');
		} else {
			...
			if ( DB_num_rows($result) != 0 ) {
				...
				$ErrMsg = _('Could not update tax');
				$result = DB_query($sql,$db, $ErrMsg);
				if (!$result){
					prnMsg(_('Tax name changed'),'success');
				}
			} else {
				$InputError = 1;
				prnMsg( _('The tax no longer exists'),'error');
			}
		}
	} elseif ($InputError !=1) {
		...
		if ( $myrow[0] > 0 ) {
			$InputError = 1;
			prnMsg( _('The tax cannot be created because another with the same name already exists'),'error');
		} else {
			...
			$ErrMsg = _('Could not add tax authority rates for the new dispatch tax. The rates of tax will not be able to be added - manual database interaction will be required to use this dispatch tax');
			$result = DB_query($sql,$db, $ErrMsg);
		}

		if (!$result){
			prnMsg(_('Errors were encountered adding this tax'),'error');
		} else {
			prnMsg(_('New tax added'),'success');
		}
	}
...
// PREVENT DELETES IF DEPENDENT RECORDS IN 'stockmaster'
	// Get the original name of the  the ID is just a secure way to find the
	$sql = "SELECT taxprovincename FROM taxprovinces
		WHERE taxprovinceid = '" . $SelectedTaxProvince . "'";
	$result = DB_query($sql,$db);
	if ( DB_num_rows($result) == 0 ) {
		// This is probably the safest way there is
		prnMsg( _('Cannot delete this tax because it no longer exists'),'warn');
	} else {
		$myrow = DB_fetch_row($result);
		$OldTaxProvinceName = $myrow[0];
		...
		$myrow = DB_fetch_row($result);
		if ($myrow[0]>0) {
			prnMsg( _('Cannot delete this tax because at least one stock location is defined to be inside this province'),'warn');
			echo '<br>' . _('There are') . ' ' . $myrow[0] . ' ' . _('stock locations that refer to this tax') . '</font>';
		} else {
			...
			$result = DB_query($sql,$db);
			prnMsg( $OldTaxProvinceName . ' ' . _('tax and any tax rates set for it have been deleted'),'success');
		}
	} //end if
	unset ($SelectedTaxProvince);
...
if (isset($SelectedTaxProvince)) {
	echo '<div class="centre"><a href=' . $_SERVER['PHP_SELF'] . '?' . SID .'>' . _('Review Tax') . '</a></div>';
}
...
if (! isset($_GET['delete'])) {
	...
	if (isset($SelectedTaxProvince)) {
		...
		$result = DB_query($sql, $db);
		if ( DB_num_rows($result) == 0 ) {
			prnMsg( _('Could not retrieve the requested tax, please try again.'),'warn');
			unset($SelectedTaxProvince);
		} else {
