	...
	if ($InputError != 1){
		...
		if (!isset($_POST['New'])) {
			$sql = "UPDATE prlpayrollperiod SET
					...
					fsyear='" . $_POST['FSYear'] . "',							
					deductsss='" . DB_escape_string($_POST['NSSF']) . "',
					deductPAYE='" . DB_escape_string($_POST['PAYE']) . "',
					deductphilhealth='" . DB_escape_string($_POST['NHIF']) . "'
...
?>
	...
	<?php	  
    echo '</SELECT></TD></TR><TR><TD width=200 height=20><div align="right"><b>' . _('Deduct NSSF') . ":</TD><TD><SELECT NAME='NSSF'>";
	echo '<OPTION VALUE=0>' . _('Yes');
	echo '<OPTION VALUE=1>' . _('No');
	echo '</SELECT></TD></TR>';
    echo '</SELECT></TD></TR><TR><TD width=200 height=20><div align="right"><b>' . _('Deduct Welfare') . ":</TD><TD><SELECT NAME='PAYE'>";
	echo '<OPTION VALUE=0>' . _('Yes');
	echo '<OPTION VALUE=1>' . _('No');
    echo '</SELECT></TD></TR><TR><TD width=200 height=20><div align="right"><b>' . _('Deduct NHIF') . ":</TD><TD><SELECT NAME='NHIF'>";
	echo '<OPTION VALUE=0>' . _('Yes');
	echo '<OPTION VALUE=1>' . _('No');
	echo '</SELECT></TD></TR>';
...
echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';
	echo '<CENTER><TABLE>';
		...
		$_POST['NSSF']  = $myrow['deductsss'];
		$_POST['PAYE']  = $myrow['deductPAYE'];
		$_POST['NHIF']  = $myrow['deductphilhealth'];
