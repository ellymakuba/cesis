...
<?php
  ...
  size=30   maxlength=20><input  type="submit" name="form1" value="Search"></td></tr>';
echo '<tr>';
if($administrator_rights !=8){
echo '<th>'._('Action').'</th>';
}
echo '<th>'._('Class').'</th></tr>';
...
$sql = "SELECT ab.*,gl.grade_level,t.title FROM autobilling ab
INNER JOIN gradelevels gl ON gl.id=ab.class_id
INNER JOIN terms t ON t.id=ab.term_id
...
while ($row = DB_fetch_array($result))
{
	echo '<td>';
	if($administrator_rights ==8){
	echo '<a  href="' . $rootpath . '/FeeStructure.php?feeStructure='
	. $row['id'].'&class='.$row['class_id'].'">'.$row['grade_level'].'-'.$row['title'].'</a>';
	}
	?></td><?php
	echo "</tr>";
	$j++;
