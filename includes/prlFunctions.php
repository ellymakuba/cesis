...
Function GetMyTax($MyTaxableIncome, $db){
	if ($MyTaxableIncome>0) {
		$sql = "SELECT rangefrom,rangeto,fixtaxableamount,fixtax,percentofexcessamount
				...
				$MyTaxAmt=$MyFixTax+(($MyTaxableIncome-$myrow['fixtaxableamount'])*($myrow['percentofexcessamount']/100))-1162;
				if($MyTaxAmt<0)
				{
				$MyTaxAmt=0;
				}
...
Function GetPAYEEE($GrossIncome, $db){
	$sql = "SELECT rangefrom,rangeto,dedtypeee,employeeshare
			...
			if ($myrow['dedtypeee']=='Fixed') {
				$MyPAYEAmt= $myrow['employeeshare'];
			} elseif ($myrow['dedtypeee']=='Percentage') {
				$MyPAYEAmt=$GrossIncome * ($myrow['employeeshare']/100);
			} else {
				$MyPAYEAmt= 0;
			}	
		    return $MyPAYEAmt;
...
Function GetPAYEER($GrossIncome, $db){
	$sql = "SELECT rangefrom,rangeto,dedtypeer,employershare
			...
			if ($myrow['dedtypeer']=='Fixed') {
				...
				$MyPAYEAmt=$GrossIncome * ($myrow['employershare']/100);
			} else {
				$MyPAYEAmt= 0;
			}	
		    return $MyPAYEAmt;
...
/*Gets the GL Codes relevant to the stock item account from the stock category record */
		...
		$sql = "SELECT payrollid,payrolldesc,payperiodid,startdate,enddate,fsmonth,fsyear,deductsss,deductPAYE,deductphilhealth,payclosed
...
Function GetEmpRow($EmpID, $db,$EmpRow){
		$sql = "SELECT paytype,payperiodid,periodrate,hourlyrate,marital,taxstatusid,employmentid,active,ssnumber,PAYEnumber,phnumber,taxactnumber,atmnumber
			...
			if ($EmpRow==21) return $myrow['PAYEnumber'];
...
Function getBankName($EmpID, $db){
		$sql = "SELECT bankname
			FROM prlemployeemaster
			WHERE employeeid= '$EmpID'";
			$result = DB_query($sql, $db);
			$myrow = DB_fetch_array($result);
            return $myrow['bankname'];
}

Function getBankCode($EmpID, $db){
		$sql = "SELECT bankcode
			FROM prlemployeemaster
			WHERE employeeid= '$EmpID'";
			$result = DB_query($sql, $db);
			$myrow = DB_fetch_array($result);
            return $myrow['bankcode'];
}
