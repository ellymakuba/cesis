...
/*PDFStarter.php has all the variables for page size and width set up depending on the users default preferences for paper size */
	...
	foreach ($_POST['Account'] as $SelectedAccount){
		...
		$FirstPeriodSelected = FormatDateForSQL($_POST['TransDate']);
		if ($_POST['tag']==0) {
	 		$sql= "SELECT type,
				      typename,
				      gltrans.typeno,
		...
		$YPos -=$line_height;
		$LeftOvers = $pdf->addTextWrap($Left_Margin,$YPos,300,$FontSize,$SelectedAccount . ' - ' . $AccountName . ' ' . ': ' . _('Listing for Date'). ' ' . $FirstPeriodSelected );
		...
		if ($PandLAccount==True) {
			...
			$sql = "SELECT Sum(amount) as amount
				FROM gltrans
				WHERE trandate<'".FormatDateForSQL($_POST['TransDate'])."'
			...
			$RunningTotal =$ChartDetailRow['amount'];
			$YPos -=$line_height;
		...
		if ($PandLAccount==True){
			...
			$LeftOvers = $pdf->addTextWrap($Left_Margin,$YPos,150,$FontSize, _('Balance C/D'));
		}
		if ($RunningTotal < 0){
		   $LeftOvers = $pdf->addTextWrap(210,$YPos,50,$FontSize, number_format(-$RunningTotal,2) , 'right');
		} else { //its a debit balance b/fwd
...
function NewPageHeader () {
	...
	$LeftOvers = $pdf->addTextWrap(130,$YPos,50,$FontSize,_('Date'),'centre');
	$LeftOvers = $pdf->addTextWrap(190,$YPos,50,$FontSize,_('Debit'),'centre');
	$LeftOvers = $pdf->addTextWrap(240,$YPos,50,$FontSize,_('Credit'),'centre');
