...
function tep_get_subjects_stream($class,$period,$db) {
	$sql = "select gl.lower from gradelevels gl,registered_students rs ,classes cl
	WHERE rs.class_id='$class'
	AND gl.id=rs.yos
...
class bus_report_stream {
	...
	function get_scheduled_students_stream($class,$period,$db) {
		...
		$sql = "select rs.id,dm.name, rs.student_id,tsr.rank,h.initial from registered_students rs
		...
		AND tsr.period_id='$period'
		AND sm.period_id='$period'
		GROUP BY rs.student_id
		ORDER BY tsr.rank asc";
		//echo $query;
		$result = DB_query($sql,$db);
