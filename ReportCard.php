...
$PageSecurity = 2;
if(isset($_POST['period_id']) && isset($_POST['student_id']) && isset($_POST['PrintPDF'])){
...
foreach ($bus_report->scheduled_subjects as $a => $b) {
	...
	$scheduled->set_calendar_vars($b['id'],$b['subject_id'],$_POST['student_id'],$_POST['period_id'],$db);
...
$XPos3=220;
foreach ($status_array as $r => $s) {
$total_marks=mode_marks($_POST['student_id'],$_POST['period_id'],$s['id'],$db);
$position=get_student_position($_POST['student_id'],$_POST['period_id'],$s['id'],$db);
...
foreach ($forms_array as $farr => $form) {
	...
	$sql = "SELECT tcr.mean,tcr.class_id,tcr.class_rank,cm.mean,cp.id FROM termly_class_ranks tcr
		...
		//$class_count=get_tracking_out_of($term['id'],$class,$db);
		...
		AND cp.term_id='". $term['id']."'
		AND tcr.class_id ='". $form['id']."'
		AND tcr.student_id='". $_POST['student_id']."'";
        $result=DB_query($sql,$db);
		$myrow=DB_fetch_row($result);
