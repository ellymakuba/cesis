...
if (isset($_POST['submit'])) {
	...
	zip,state,mobileno,grade_level_id,class_id,relationship,gname,gboxno,gstate,gmobileno,email,group_id,final_grade,transport,estate_id,dob,disease,pschool,house,index_no,balance)
	VALUES ('" . $_POST['DebtorNo'] ."','" . $_POST['CustName'] ."','" . $_POST['age'] ."',
'" . $_POST['gender'] ."','" . $_POST['boxno'] . "','" . $_POST['town'] . "','" . $_POST['zip'] . "',
'" . $_POST['state'] . "','" . $_POST['mobileno'] . "','" . $stream_class . "','" . $_POST['student_class'] . "',
'" . $_POST['relationship']. "','" . $_POST['gname'] . "','" . $_POST['gboxno']. "','" . $_POST['gstate'] . "',
'" . $_POST['gmobileno'] . "','" . $_POST['Email'] . "','" . $_POST['student_group'] . "','" . $_POST['final_grade'] . "','" . $_POST['transport'] . "','" . $_POST['estate'] . "','" . $_POST['dob'] . "','" . $_POST['disease'] . "','" . $_POST['pschool'] . "','" . $_POST['house'] . "','" . $_POST['index_no'] . "','" . $_POST['balance'] . "')";
...
if (!isset($id)) {
	...
	$DataError =0;
	echo '<table class=enclosed><tr><td class="visible"><table>';
	echo '<tr><td colspan="2" class="visible"><h3>General Information</h3></td></tr>';
	if ($_SESSION['AutoDebtorNo']==0)  {
		echo '<tr><td class="visible">' . _('Student AdmNo') . ":</td><td class=\"visible\">
		<input tabindex=1 type='Text' name='DebtorNo' size=30 maxlength=30></td></tr>";
	}
...
'</SELECT></TD></TR>';
  echo '</table></td><td class="visible"><table>';
...
//editing student exists - either passed when calling the form or from the form itself
	echo "<form method='post' action='" . $_SERVER['PHP_SELF'] . '?' . SID ."'>";
	echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';
	echo '<table class=enclosed><tr><td valign=top class="visible"><table>';
	if (!isset($_POST['New'])) {
	$sql = "SELECT *
	FROM debtorsmaster
	WHERE debtorsmaster.id = '" . $id . "'";
...
DB_data_seek($result3,0);
	echo '</select></td>';
		...
		echo '<tr><td class="visible">' . _('Student AdmNo') . ':</td>
			<td class="visible"><input ' . (in_array('DebtorNo',$Errors) ?  'class="inputerror"' : '' ) .' type="Text" name="DebtorNo" value="' . $_POST['DebtorNo'] . '" size=30 maxlength=40 readonly></td></tr>';
echo '<tr><td class="visible">' . _('Student Name') . ':</td>
...
echo '<tr><td class="visible">' . _('Stream').":</td><td colspan=\"2\" class=\"visible\"><select name='student_class'>";
$sql2="SELECT class_id FROM debtorsmaster WHERE id='$id'";
$result2=DB_query($sql2,$db);
$myrow2 = DB_fetch_array($result2);
$current_class_id=$myrow2['class_id'];
