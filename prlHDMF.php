...
if (isset($_POST['submit'])) {
	...
	if ($InputError !=1) {
			...
			$sql = "UPDATE prlhdmftable SET 
					...
					employershare='" .DB_escape_string($_POST['ERPAYE']) . "', 
					dedtypeee='" .$_POST['DedTypeEE']. "', 
					employeeshare='" .DB_escape_string($_POST['EEPAYE']) . "'
			...
			prnMsg(_('The PAYE master record for') . ' ' . $Bracket . ' ' . _('has been updated'),'success');
		...
		$sql = "INSERT INTO prlhdmftable (bracket, 
				 ...
				 VALUES ('$Bracket', 
						...
						'" .$_POST['DedTypeER']. "', 
						'" .DB_escape_string($_POST['ERPAYE']) . "', 
						'" .$_POST['DedTypeEE']. "', 
						'" .DB_escape_string($_POST['EEPAYE']) . "')"; 
...
// PREVENT DELETES IF DEPENDENT RECORDS found
	if ($CancelDelete == 0) {
		...
		prnMsg(_('PAYE record for') . ' ' . $Bracket . ' ' . _('has been deleted'),'success');
		unset($Bracket);
...
echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';
	...
	echo "<TD><INPUT TYPE='text' NAME='ERPAYE' SIZE=14 MAXLENGTH=12></TD>";
	echo '</SELECT></TD></TR><TR><TD>' . _('Employee Share') . ":</TD><TD><SELECT NAME='DedTypeEE'>";
	echo '<OPTION VALUE="Fixed">' . _('Fixed');
	echo '<OPTION VALUE="Percentage">' . _('Percentage');
	echo "<TD><INPUT TYPE='text' NAME='EEPAYE' SIZE=14 MAXLENGTH=12></TD>";
	echo "</SELECT></TD></TR></TABLE><p><CENTER><INPUT TYPE='Submit' NAME='submit' VALUE='" . _('Insert New PAYE') . "'>";
	echo '</FORM>';
...
echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';
	...
	if (!isset($_POST['New'])) {
		...
		$_POST['ERPAYE']  = $myrow['employershare'];
		$_POST['DedTypeEE']  = $myrow['dedtypeee'];
		$_POST['EEPAYE']  = $myrow['employeeshare'];
	...
	echo "<TD><INPUT TYPE='text' NAME='ERPAYE' SIZE=14 MAXLENGTH=12 value='" . $_POST['ERPAYE'] . "'></TD>";
	...
	echo "<TD><INPUT TYPE='text' NAME='EEPAYE' SIZE=14 MAXLENGTH=12 value='" . $_POST['EEPAYE'] . "'></TD>";
