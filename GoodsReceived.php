...
/*Now the SQL to do the update to the PurchOrderDetails */
			...
			if ($OrderLine->ReceiveQty >= ($OrderLine->Quantity - $OrderLine->QtyReceived)){
				$SQL = "UPDATE purchorderdetails SET
							...
							total_amount=unitprice*(quantityrecd + '" . $OrderLine->ReceiveQty . "'),
							completed=1
...
/* Need to get the current location quantity will need it later for the stock movement */
				...
				$SQL = "UPDATE stockmaster
					SET quantity = quantity + '" . $OrderLine->ReceiveQty . "'
					WHERE stockid = '" . $OrderLine->StockID . "'";

				$ErrMsg =  _('CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR AND SEEK ASSISTANCE') . ': ' . _('Product quantity failed to update');
				$DbgMsg =  _('The following SQL to update the quantity record was used');
				$Result = DB_query($SQL, $db, $ErrMsg, $DbgMsg, true);
...
/* If GLLink_Stock then insert GLTrans to debit the GL Code  and credit GRN Suspense account at standard cost*/
			if ($_SESSION['PO']->GLLink==1){ /*GLCode is set to 0 when the GLLink is not activated this covers a situation where the GLLink is now active but it wasn't when this PO was entered */
						...
						VALUES (
							...
							'PO: " . $_SESSION['PO']->OrderNo . " " . $_SESSION['PO']->SupplierID . " - " . $OrderLine->StockID
									. " - " . $OrderLine->ItemDescription . " x " . $OrderLine->ReceiveQty . " @ " .
										number_format($OrderLine->Price,2) . "',
							'" . $OrderLine->Price * $OrderLine->ReceiveQty . "'
							)";
						...
						VALUES (25,
							...
							'" . _('PO') . ': ' . $_SESSION['PO']->OrderNo . ' ' . $_SESSION['PO']->SupplierID . ' - ' .
										$OrderLine->StockID . ' - ' . $OrderLine->ItemDescription . ' x ' .
											$OrderLine->ReceiveQty . ' @ ' . number_format($OrderLine->Price,2) . "',
							'" . -$OrderLine->Price * $OrderLine->ReceiveQty . "'
							)";
