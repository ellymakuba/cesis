...
/* $Id: GLJournal.php 4087 2010-10-02 11:37:38Z tim_schofield $*/
/* $Revision: 1.27 $ */
	...
	$AllowThisPosting = true; //by default
		if ($_SESSION['ProhibitJournalsToControlAccounts'] == 1){
			if ($_SESSION['CompanyRecord']['gllink_debtors'] == '1' AND $_POST['GLManualCode'] == $_SESSION['CompanyRecord']['debtorsact']){
				prnMsg(_('GL Journals involving the debtors control account cannot be entered. The general ledger debtors ledger (AR) integration is enabled so control accounts are automatically maintained by AIRADS System. This setting can be disabled in System Configuration'),'warn');
			...
			if ($_SESSION['CompanyRecord']['gllink_creditors'] == '1' AND $_POST['GLManualCode'] == $_SESSION['CompanyRecord']['creditorsact']){
				prnMsg(_('GL Journals involving the creditors control account cannot be entered. The general ledger creditors ledger (AP) integration is enabled so control accounts are automatically maintained by AIRADS System. This setting can be disabled in System Configuration'),'warn');
				$AllowThisPosting = false;
			}
		}
...
if (!Is_Date($_SESSION['JournalDetail']->JnlDate)){
	...
	echo '<table><tr>
			...
			<td><input type='text' class='date' alt='".$_SESSION['DefaultDateFormat']."' name='JournalProcessDate' maxlength=10 size=11></td>";
	echo '<td>' . _('Type') . ':</td>
