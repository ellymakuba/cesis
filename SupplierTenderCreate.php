...
if (isset($_POST['NewItem']) and !isset($_POST['Refresh'])) {
	foreach ($_POST as $key => $value) {
		if (mb_substr($key,0,7)=='StockID') {
			...
			$UOM = $_POST['UOM'.$Index];
			$sql="SELECT description, decimalplaces FROM stockmaster WHERE id='".$StockID."'";
			$result=DB_query($sql, $db);
			$myrow=DB_fetch_array($result);
			$_SESSION['tender'.$identifier]->add_item_to_tender($_SESSION['tender'.$identifier]->LinesOnTender,
																$StockID,
	...
	if (isset($SearchResult)) {
		...
		while ($myrow=DB_fetch_array($SearchResult)) {
			...
			echo '<td>'.$myrow['stockid'].'</td>
					...
					<input type="hidden" value="'.$myrow['id'].'" name="StockID'.$i.'" />
					<input type="hidden" value="'.$myrow['description'].'" name="ItemDescription'.$i.'" />
					</tr>';
