...
if (isset($_POST['submit'])) {
	...
	if ($InputError !=1) {
			...
			prnMsg(_('The NSSF master record for') . ' ' . $Bracket . ' ' . _('has been updated'),'success');
		...
		$sql = "INSERT INTO prlsstable (bracket, 
			...
			$ErrMsg = _('The NSSF') . ' ' . $_POST['Credit'] . ' ' . _('could not be added because');
...
// PREVENT DELETES IF DEPENDENT RECORDS IN 'SuppTrans' , PurchOrders, SupplierContacts
	if ($CancelDelete == 0) {
		...
		prnMsg(_('NSSF record for') . ' ' . $Bracket . ' ' . _('has been deleted'),'success');
		unset($Bracket);
...
//	echo '</SELECT></TD></TR>';
	echo "</SELECT></TD></TR></TABLE><p><CENTER><INPUT TYPE='Submit' NAME='submit' VALUE='" . _('Insert New NSSF') . "'>";
	echo '</FORM>';
...
echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';
	...
	// its a new NSSF being added
		echo "<INPUT TYPE=HIDDEN NAME='New' VALUE='Yes'>";
		echo '<TR><TD>' . _('NSSF Code') . ":</TD><TD><INPUT TYPE='text' NAME='Bracket' VALUE='$Bracket' SIZE=5 MAXLENGTH=4></TD></TR>";
	}
	...
	if (isset($_POST['New'])) {
		echo "</TABLE><P><CENTER><INPUT TYPE='Submit' NAME='submit' VALUE='" . _('Add These New NSSF Details') . "'></FORM>";
	} else {
		echo "</TABLE><P><CENTER><INPUT TYPE='Submit' NAME='submit' VALUE='" . _('Update NSSF') . "'>";
