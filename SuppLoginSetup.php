<?php
/* $Revision: 1.2 $ */
/* $Id: SuppLoginSetup.php 4009 2010-09-30 16:03:36Z tim_schofield $*/
...
echo '<p class="page_title_text"><img src="'.$rootpath.'/css/'.$theme.'/images/supplier.png" title="' . _('Supplier') . '" alt="">' . ' ' . _('Supplier') . ' : ' . $_SESSION['SupplierID'] . _(' has been selected') . '<br>';//'</p>';
...
/* If its the first time the page has been displayed with no parameters then none of the above are true and the list of Users will be displayed with links to delete or edit each. These will call the same page again and allow update/input or deletion of the records*/
	...
	while ($myrow = DB_fetch_array($result)) {
		...
		echo "<td>".$myrow['userid']."</td>
			...
			<td><a href=\"".$_SERVER['PHP_SELF']  . "?" . SID."&SelectedUser=".$myrow[0]."\">" . _('Edit') . "</a></td>
			<td><a href=\"".$_SERVER['PHP_SELF'] . "?" . SID."&SelectedUser=".$myrow[0]."&delete=1\">" . _('Delete') . "</a></td>
			</tr>";
