...
$sql="SELECT * FROM gradelevels ORDER BY grade_level";
...
if(isset($_GET['feeStructure']))
{
	...
	$_SESSION['term'] = $myrow['term_id'];
}
if((isset($_SESSION['classSession']) && isset($_SESSION['term'])))
{
...
if(isset($_GET['feeStructure']))
{
	...
	while($rows=DB_fetch_array($result))
	{
		...
		$_SESSION['InvoiceItems']->add_to_cart ($myrow2['stockid'],$NewItemQty,$myrow2['description'],$rows['amount']
...
if (count($_SESSION['InvoiceItems']->LineItems)>0)
{
  ...
  $class_session = $myrow['grade_level'];
	...
	$LineNumber = $_SESSION['InvoiceItems']->LineCounter;
	$sql = 'SELECT * FROM terms WHERE id="'.$_SESSION['term'].'"';
	$result=DB_query($sql,$db);
	$myrow=DB_fetch_array($result);
	$term=$myrow['title'];
...
echo '<tr><td colspan="6" style="text-align:center;"><h1><b>'.$class_session.' '.$term.' '._('Fee Structure') . '</b></h3></td></tr>';
...
if(isset($_POST['submitInvoice'])){
		...
		$sql = "INSERT INTO autobilling (class_id,term_id)
		VALUES ('" .$_SESSION['classSession'] ."','" .$_SESSION['term'] ."') ";
