<?php
/* $Id: SupplierTenders.php 4837 2012-01-24 23:41:46Z vvs2012 $*/
...
echo '<tr><td>' . _('Select Supplier') . ': </td><td><select tabindex="5" name="SupplierID">';
echo '<OPTION SELECTED VALUE=0>' . _('Select Period');
$result = DB_query('SELECT * FROM suppliers',$db);
while ($myrow = DB_fetch_array($result)) {
	...
	echo $myrow['supplierid'] . '>' . $myrow['suppname'];
} //end while loop
echo '</select></td></tr>';
echo '</table>';
echo "<br><div class='centre'><input tabindex=20 type='Submit' name='selectsupplier' value='" . _('Submit') . "'>&nbsp;<input tabindex=21 type=submit action=RESET VALUE='" . _('Reset') . "'></div>";
echo '</form>';
if (isset($_POST['selectsupplier'])) {
$sql="SELECT supplierid FROM suppliers WHERE supplierid='" . $_POST['SupplierID'] . "'";
...
if (isset($_POST['TenderType'])
	...
	while ($myrow1 = DB_fetch_array($result)) {
		if ($myrow1['id'] == $_POST['StockCat']) {
			echo '<option selected="selected" value="' . $myrow1['id'] . '">' . $myrow1['categorydescription'] . '</option>';
		} else {
			echo '<option value="' . $myrow1['id'] . '">' . $myrow1['categorydescription'] . '</option>';
		}
	}
	echo '</select>';
	...
	if ($_POST['Keywords']) {
		...
		if ($_POST['StockCat']=='All'){
			$sql = "SELECT stockmaster.stockid,
					...
					ON stockmaster.categoryid=stockcategory.id
