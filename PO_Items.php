			...
			if ($AlreadyOnThisOrder!=1 and $Quantity>0){
				...
				if ($myrow[0]>0) {
					$sql = "SELECT stockmaster.description,
						...
						stockmaster.actualcost as price,
						purchdata.conversionfactor,
						purchdata.supplierdescription,
					...
					WHERE chartmaster.accountcode = stockcategory.stockact
						AND stockcategory.id = stockmaster.categoryid
...
if (!isset($_GET['Edit'])) {
	...
	while ($myrow1 = DB_fetch_array($result1)) {
		if (isset($_POST['StockCat']) and $_POST['StockCat']==$myrow1['id']){
			echo "<option selected value=". $myrow1['id'] . '>' . $myrow1['categorydescription'];
		} else {
			echo "<option value=". $myrow1['id'] . '>' . $myrow1['categorydescription'];
		}
	}
