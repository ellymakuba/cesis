...
/* $Id: Payments.php 3917 2010-09-30 15:06:25Z tim_schofield $*/
/* $Revision: 1.39 $ */
...
$LineItemsSQL = "SELECT purchorderdetails.* FROM purchorderdetails
				WHERE purchorderdetails.orderno = '" . $_SESSION['orderno'] ."'";
...
echo '<p class="page_title_text"><img src="'.$rootpath.'/css/'.$theme.'/images/transactions.png" title="' . _('Supplier Payment Entry')
	. '" alt="">' . ' ' . _('Supplier Payment Entry') . '</p>';
...
$sql = "SELECT p.orderno,
	...
	FROM 
		purchorders p
	WHERE 
		p.supplierno ='".$_GET['SupplierID']."'
		AND p.orderno='".$_SESSION['orderno']."'
	ORDER BY 
		p.orderno DESC;";
...
echo '<tr><td>' . _('Order No') . ':</td>
	<td><input type="text" name="orderno"  value="' . $_SESSION['orderno'] . '" readonly=""></td>
	</tr>';
...
echo '<tr><td>' . _('Tender Total Amount') .':</td><td>'. $OrderTotal.'</td>';
echo '<tr><td>' . _('Tender Amount Paid already') .':</td><td>'. $suppliertotalpayment.'</td>';
